# StarRocks Railway Template — Design
...
4. **Root password.** Marker file `/var/lib/starrocks/.root_password_set` on the volume.
...
|----------|---------|-------------|
| `STARROCKS_PASSWORD` | `${{secret(32)}}` | Root password, applied on first boot |
