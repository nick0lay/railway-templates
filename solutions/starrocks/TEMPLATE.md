...
- **Secured by default**: the `root` password is generated by Railway at install time and applied to the account on first boot
...
First boot takes a few minutes — the image is large and the cluster registers its backend before accepting connections. The service is ready when the logs read `cluster initialization DONE!`.
